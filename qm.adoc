...
* the *NVME* controller, You can connect up to 6 devices on this controller.
...
* *ramfb*, is a qemu ramfb adapter.
* *mdev*, is a vga display function by vfio-pci.Selecting this will enable vgpu device display theri fb.Support nvidia & mthreads vgpu.
...
For arm64/riscv64/loongarch64 device , must use *OVMF* to boot os.
...
PXVIRT has two templates. pxvditemplate && template. The Pxvditemplate type can be started. This is often used in vdi scenarios.
